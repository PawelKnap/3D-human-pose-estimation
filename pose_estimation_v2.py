...
def tilt(poses):
    ...
    for i in range(len(poses)):
        ...
        #    print('left hand elbow rise higher than neck')
        ...
        #    print('right hand elbow rise higher than neck')
            polar_dis[i, 13] = polar_dis[i, 13] + s[i]
            polar_dis[i, 14] = polar_dis[i, 14] + s[i]
        #spher[i, 7:15, 0] = spher[i, 7:15, 0] + s[i]
...
def radar_data_transform(xzs):
    ...
    if xzs[0] is None and xzs[1] is None and xzs[2] is None and xzs[3] is None:
        ...
        if xzs[0] is not None:
            coords_for_better = np.ones(shape=(xzs[0].shape[0], 3))
            for i in range(xzs[0].shape[0]):
                coords_for_better[i] = [xzs[0][i][0],xzs[0][i][1],1]
            transformed_1 = (radar_1_transform @ coords_for_better.T).T
            coords_better = (intrinsic_par_1 @ coords_for_better.T).T
...
if __name__ == '__main__':
    ...
    try:
        ...
        mtx, dist = load_coefficients('calibration_chessboard.yml') #camera calibration coefficients
        ...
        while True:
            ...
            #print('Radars ',xzs)
            coords, transformed = radar_data_transform(xzs) #transform radar data
            datum = op.Datum()
            ...
            if datum.poseKeypoints is not None and coords is not None: #if OpenPose detects a person
                ...
                if len(index) != 0: #If more than one individual was matched then calculate its global 3D coordinates and display them
                    if k == 0: #could be done even simpler!
                        coord_storage = coords[index]
                        dis_angle = np.full([2,coords[index].shape[0], 7], np.nan) #or len(index)
                        ...
                        #print('aaaa',radar_coords_f,'a', dis_angle,'a', radar_coords_no_pair,'a', dis_angle_no_pair)
                        k = k + 1
                    else:
